...
class Polynomial:
    ...
    def __add__(self, rhValue):
        if isinstance(rhValue, Polynomial):
            ...
            if not isinstance(rhValue, (int, float)):
                raise TypeError("Incorrect type of value. 'int' or 'float' type is expected.")
    ...
    def __eq__(self, rh):
        if not isinstance(rh, (Polynomial, int, float)):
            raise TypeError("Incorrect type of input data. Polynomial or int or float is expected.")
        if isinstance(rh, Polynomial):
            return self.coeffs == rh.coeffs
        else:
            if len(self.coeffs) != 1:
                return False
            else:
                return self.coeffs[0] == rh
    ...
    def checkObjectsTypeCorrectness(self, coeffsList):
        for i, coeff in enumerate(coeffsList):
            if not isinstance(coeff, (int, float)):
                raise TypeError("The polynomial coefficients must have 'int' or 'float' type.")
